...
class iVAE(nn.Module):
    def __init__(self, prior, posterior, likelihood, z_dim, nclasses, dset, activation, device, n_layers, slope=.2,
                 nc=3):
        ...
        if dset == 'synth':
            self.encoder = nets.MLP(self.x_dim, self.post_dim, z_dim * 10, n_layers, activation=activation, slope=slope,
                                    device=device)
            self.decoder = nets.MLP(z_dim, self.x_dim, z_dim * 10, n_layers, activation=activation, slope=slope,
                                    device=device)
        ...
        if self.prior.name != 'uniform':
            self.conditioner = nets.MLP(nclasses, z_dim * 2, z_dim * 4, n_layers, activation=activation, slope=slope,
                                        device=device)
