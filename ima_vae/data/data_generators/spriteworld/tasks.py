...
class FindGoalPosition(AbstractTask):
    ...
    def _single_sprite_reward(self, sprite):
        goal_distance = (
            np.sum(
                self._weights_dimensions
                * (sprite.position - self._goal_position) ** 2.0
            )
            ** 0.5
        )
    ...
    def _filtered_sprites_rewards(self, sprites):
        ...
        rewards = [
            self._single_sprite_reward(s)
            for s in sprites
            if self._filter_distrib is None or self._filter_distrib.contains(s.factors)
        ]
        return rewards
...
class Clustering(AbstractTask):
    ...
    def reward(self, sprites):
        ...
        dense_reward = (metric - self._termination_threshold) * self._reward_range / 2.0
