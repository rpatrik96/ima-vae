...
def to_one_hot(x, m=None):
    ...
    return xoh
nfactors = 4
beta_params = torch.Tensor(np.random.uniform(args.lower,args.upper,2*nfactors*args.nclasses)).view(args.nclasses,nfactors,2).numpy()
angle_params = torch.zeros((args.nclasses,2)).numpy()
shape_probs = torch.zeros((args.nclasses,3)).numpy()

sname_train = "isprites_train_" + "nclasses_" + str(args.nclasses) + "_nobs_" + str(args.nobs) + "_lower_" + str(args.lower) + "_upper_" + str(args.upper)
sname_val = "isprites_val_" + "nclasses_" + str(args.nclasses) + "_nobs_" + str(args.nobs) + "_lower_" + str(args.lower) + "_upper_" + str(args.upper)
if args.angle:
  nfactors+=1
  sname+="_angle"
if args.shape:
